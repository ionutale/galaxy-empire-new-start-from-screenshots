                    ...
                    <form method="POST" action="?/research" use:enhance>
                        ...
                        <button 
                            ...
                            class="w-full py-2 rounded font-bold transition-colors
                                {data.resources.metal >= cost.metal && data.resources.crystal >= cost.crystal && data.resources.gas >= cost.gas
                                    ? 'bg-green-600 hover:bg-green-500 text-white' 
                                    : 'bg-gray-600 text-gray-400 cursor-not-allowed'}"
                            disabled={!(data.resources.metal >= cost.metal && data.resources.crystal >= cost.crystal && data.resources.gas >= cost.gas)}
                        >
                            Research
                        </button>
                    </form>
