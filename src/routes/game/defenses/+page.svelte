...
<div class="p-4 pb-20">
    ...
    {#if data.shipyardLevel === 0}
        <div class="bg-red-900/50 border border-red-500 p-4 rounded text-center text-red-200 mb-6">
            You need a Shipyard to build defenses. <a href="/game/facilities" class="underline font-bold hover:text-white">Build one in the Facilities menu.</a>
        </div>
    {/if}
            ...
            <div class="bg-gray-800 border border-gray-700 rounded-lg p-4 flex flex-col shadow-lg">
                ...
                <div class="mt-auto">
                    ...
                    <form method="POST" action="?/build" use:enhance class="flex space-x-2">
                        ...
                        {:else}
                            <input type="number" name="amount" min="1" value="1" class="w-16 bg-gray-700 border border-gray-600 rounded px-2 py-1 text-white text-center" disabled={data.shipyardLevel === 0}>
                            <button 
                                type="submit"
