...
export function simulateCombat(attackerFleet: Record<string, number>, defenderFleet: Record<string, number>, defenderDefenses: Record<string, number>): CombatResult {
    ...
    let defenderUnits = [
        ...
        ...Object.entries(defenderDefenses).map(([type, count]) => {
            ...
            return { type, count, attack: stats.attack, defense: stats.defense, shield: stats.shield, isShip: false };
        })
    ];
    ...
    const defenderPower = defenderUnits.reduce((sum, u) => sum + (u.attack * u.count) + (u.shield * u.count / 10), 0); // Shield helps a bit
