import { db } from './db';
import { fleets, planets, planetResources, planetShips, planetDefenses, messages, users } from './db/schema';
...
export async function processFleets() {
    ...
    try {
        await db.transaction(async (tx) => {
            ...
            const fleetsRes = await tx.select()
                .from(fleets)
                .where(and(
...
async function processArrivingFleet(tx: any, fleet: any) {
    // Find target planet
    const targetRes = await tx.select({ id: planets.id, userId: planets.userId })
        .from(planets)
        .where(and(
    ...
    if (fleet.mission === 'transport') {
        ...
        if (!targetPlanet) {
            // Create new colony
            const newPlanetRes = await tx.insert(planets).values({
                userId: fleet.userId,
        ...
        if (targetPlanet && targetPlanet.userId) {
            ...
            const defShipsRes = await tx.select().from(planetShips).where(eq(planetShips.planetId, targetPlanet.id));
            const defDefensesRes = await tx.select().from(planetDefenses).where(eq(planetDefenses.planetId, targetPlanet.id));
