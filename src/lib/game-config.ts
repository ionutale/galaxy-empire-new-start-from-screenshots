...
export function getBuildingCost(type: string, level: number): Cost | null {
	...
	return {
		...
		gas: building.baseCost.gas
			? Math.floor(building.baseCost.gas * Math.pow(building.costFactor, level))
			: 0,
		energy: building.baseCost.energy
			? Math.floor(building.baseCost.energy * Math.pow(building.costFactor, level))
			: 0
	};
}
...
export function getResearchCost(type: string, level: number): Cost | null {
	...
	return {
		...
		energy: tech.baseCost.energy
			? Math.floor(tech.baseCost.energy * Math.pow(tech.costFactor, level))
			: 0
	};
}
