import { pgTable, unique, serial, varchar, text, integer, boolean, timestamp, foreignKey, index, jsonb, doublePrecision, primaryKey } from "drizzle-orm/pg-core"
...
export const planets = pgTable("planets", {
	...
	unique("planets_galaxy_id_system_id_planet_number_unique").on(table.galaxyId, table.systemId, table.planetNumber),
]);
...
export const fleets = pgTable("fleets", {
	...
	index("fleets_origin_planet_id_idx").using("btree", table.originPlanetId.asc().nullsLast().op("int4_ops")),
	index("fleets_status_arrival_time_idx").using("btree", table.status.asc().nullsLast().op("timestamp_ops"), table.arrivalTime.asc().nullsLast().op("text_ops")),
...
export const messages = pgTable("messages", {
	...
	index("messages_user_id_created_idx").using("btree", table.userId.asc().nullsLast().op("int4_ops"), table.createdAt.asc().nullsLast().op("timestamp_ops")),
	index("messages_user_id_idx").using("btree", table.userId.asc().nullsLast().op("int4_ops")),
	index("messages_user_id_read_idx").using("btree", table.userId.asc().nullsLast().op("int4_ops"), table.isRead.asc().nullsLast().op("bool_ops")),
	foreignKey({
			columns: [table.userId],
			foreignColumns: [users.id],
...
export const researchQueue = pgTable("research_queue", {
	...
	index("research_queue_completion_idx").using("btree", table.completionAt.asc().nullsLast().op("timestamp_ops")),
...
export const userResearchLevels = pgTable("user_research_levels", {
	...
	index("user_research_levels_researching_idx").using("btree", table.userId.asc().nullsLast().op("int4_ops"), table.isResearching.asc().nullsLast().op("int4_ops")),
	index("user_research_levels_user_research_idx").using("btree", table.userId.asc().nullsLast().op("int4_ops"), table.researchTypeId.asc().nullsLast().op("int4_ops")),
	foreignKey({
			columns: [table.userId],
			foreignColumns: [users.id],
...
export const shipyardQueue = pgTable("shipyard_queue", {
	...
	index("shipyard_queue_completion_idx").using("btree", table.completionAt.asc().nullsLast().op("timestamp_ops")),
	index("shipyard_queue_planet_idx").using("btree", table.planetId.asc().nullsLast().op("int4_ops")),
...
export const planetBuildings = pgTable("planet_buildings", {
	...
	unique("planet_buildings_planet_id_building_type_id_key").on(table.planetId, table.buildingTypeId),
]);
...
export const privateMessages = pgTable("private_messages", {
	...
	index("private_messages_created_idx").using("btree", table.createdAt.asc().nullsLast().op("timestamp_ops")),
	index("private_messages_from_to_idx").using("btree", table.fromUserId.asc().nullsLast().op("int4_ops"), table.toUserId.asc().nullsLast().op("int4_ops")),
	index("private_messages_from_user_idx").using("btree", table.fromUserId.asc().nullsLast().op("int4_ops")),
	index("private_messages_to_user_idx").using("btree", table.toUserId.asc().nullsLast().op("int4_ops")),
	foreignKey({
...
export const buildingQueue = pgTable("building_queue", {
	...
	index("idx_building_queue_completion_at").using("btree", table.completionAt.asc().nullsLast().op("timestamp_ops")),
	index("idx_building_queue_planet_id").using("btree", table.planetId.asc().nullsLast().op("int4_ops")),
	foreignKey({
			columns: [table.planetId],
			foreignColumns: [planets.id],
...
export const allianceDiplomacy = pgTable("alliance_diplomacy", {
	id: serial().primaryKey().notNull(),
...
export const userCommanders = pgTable("user_commanders", {
	...
	primaryKey({ columns: [table.userId, table.commanderId], name: "user_commanders_user_id_commander_id_pk"}),
]);
